...
export default function Header() {
  ...
  return (
    <header
      className={`flex py-4 px-4 sm:px-10 min-h-[70px] tracking-wide sticky top-0 z-[1000] transition-all duration-300 ${
        isScrolled
    ...
    >
      ...
      <div 
        ref={menuRef}
        className={`fixed top-0 left-0 h-full w-3/4 min-w-[300px] bg-white shadow-2xl z-[1001] transition-transform duration-300 lg:hidden ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex flex-col h-full p-6">
          ...
          <div className="mt-auto pt-6 border-t font-semibold">
            <Link href="/case-study/3m-india-limited" onClick={handleCloseMenu}>
              <button className="w-full py-4 bg-[#00BE5D] text-white rounded-xl font-bold shadow-lg shadow-green-100 transition-transform active:scale-95">
                Case Study
              </button>
            </Link>
          </div>
      ...
      {isMenuOpen && (
        <div 
          className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[1000] lg:hidden transition-opacity duration-300"
